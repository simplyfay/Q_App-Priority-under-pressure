...
- ANIMATION: rotating conic-gradient halo in --action-primary (#457B9D), full rotation 3s linear infinite, calm not urgent — communicates Q is actively working
...
- Three lines only. No buttons. No cards. Background: var(--surface-default) — adapts to current theme (light surface in light mode, dark surface in dark mode)
...
- Background: var(--surface-default) — adapts to current theme (light surface in light mode, dark surface in dark mode)
- "You're clear."
...
> Values below are dark-mode reference. Light-mode values are extracted from Figma and generated by /build-token-system. Light mode is the default app experience.
...
### Dark-mode contrast audit (reference only)

> Authoritative WCAG audit for both light and dark modes is run by /build-token-system after token extraction.
